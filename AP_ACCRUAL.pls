--Accrual Account >> 51411 Route Charges - AP SIDE
--Expense Account >> 21511 Accrual - Operations Related - GL SIDE
...
JEL.ACCOUNT_ID, JEL.ACCOUNT_NAME, NVL(JEL.ACCRUAL_PERIOD,'N/A') AS ACCRUAL_PERIOD,
...
GLJB.NAME AS JE_BATCH_NAME, GLJB.JE_BATCH_ID, GLJB.DEFAULT_PERIOD_NAME, --GLJB.JE_SOURCE,
...
ROUND(NVL(GLJL.ENTERED_DR,0)+NVL(GLJL.ENTERED_CR*-1,0),2) AS TRX_AMT,
GLJL.ACCOUNTED_DR,GLJL.ACCOUNTED_CR,
ROUND(NVL(GLJL.ACCOUNTED_DR,0)+NVL(GLJL.ACCOUNTED_CR*-1,0),2) AS ACCT_AMT,
...
WHERE JEL.AOC_NAME IN (:AOC_NAME)
AND JEL.ACCOUNT_ID IN (:EXPENSE_ACCT)
AND JEL.ACCT_PERIOD <= (SELECT CURR_PERIOD FROM P)
AND JEL.SEGMENT4 IN (:SEGMENT_LOC)
AND JEL.JE_SOURCE = 'Payables'
)
---AP_IL---------------------------------------
,APIL AS (
...
---GLO_SUM-----------
,GLO_SUM AS (
SELECT AOC_NAME, ACCT_PERIOD, ACCOUNT_ID, SUM(ACCT_AMT) AS ACCT_AMT FROM GLO GLO
GROUP BY AOC_NAME, ACCT_PERIOD, ACCOUNT_ID
)
---GLAP_SUM-----------
,GLAP_SUM AS (
SELECT AOC_NAME, ACCT_PERIOD, ACCOUNT_ID, SUM(ACCT_AMT) AS ACCT_AMT FROM GLAP GLAP
GROUP BY AOC_NAME, ACCT_PERIOD, ACCOUNT_ID
)

---GLO1---------------------------------------
,GLO1 AS (
SELECT GLO.AOC_NAME, GLO.AOC_ID, GLO.ACCT_YEAR, GLO.ACCT_PERIOD, GLO.ACCRUAL_PERIOD,GLO.ACCOUNT_ID, GLO.SEGMENT4,GLO.SEGMENT5, GLO.ACCOUNT_NAME,GLO.ACCT_CURR, 'ALL' AS JE_SOURCE, --GLO.JE_SOURCE,
GLO.AOC_NAME ||'-'|| GLO.ACCT_PERIOD ||'-'|| GLO.ACCRUAL_PERIOD AS COMBO_IDX,
...
SUM(GLO.ACCOUNTED_DR) AS ACCOUNTED_DR,
SUM(GLO.ACCOUNTED_CR) AS ACCOUNTED_CR
FROM GLO GLO
...
---GLAP1---------------------------------------
,GLAP1 AS (
SELECT GLAP.AOC_NAME, GLAP.AOC_ID, GLAP.ACCT_YEAR, GLAP.ACCT_PERIOD, GLAP.ACCRUAL_PERIOD,GLAP.ACCOUNT_ID, GLAP.SEGMENT4, GLAP.SEGMENT5, GLAP.ACCOUNT_NAME,GLAP.ACCT_CURR, GLAP.JE_SOURCE,
GLAP.AOC_NAME ||'-'|| GLAP.ACCT_PERIOD ||'-'|| GLAP.ACCRUAL_PERIOD AS COMBO_IDX,
ROW_NUMBER() OVER (PARTITION BY GLAP.AOC_NAME, GLAP.ACCOUNT_ID, GLAP.ACCT_PERIOD, GLAP.ACCRUAL_PERIOD ORDER BY GLAP.ACCT_PERIOD ASC) AS RNA,
ROW_NUMBER() OVER (PARTITION BY GLAP.AOC_NAME, GLAP.ACCOUNT_ID, GLAP.ACCT_PERIOD, GLAP.ACCRUAL_PERIOD ORDER BY GLAP.ACCT_PERIOD DESC) AS RND,
COUNT(GLAP.JOURNAL_NAME) AS J_COUNT,
SUM(GLAP.ACCT_AMT) AS ACCT_AMT,
SUM(GLAP.ACCOUNTED_DR) AS ACCOUNTED_DR,
SUM(GLAP.ACCOUNTED_CR) AS ACCOUNTED_CR
FROM GLAP GLAP
GROUP BY GLAP.AOC_NAME, GLAP.AOC_ID, GLAP.ACCT_YEAR, GLAP.ACCT_PERIOD, GLAP.ACCRUAL_PERIOD,GLAP.ACCOUNT_ID, GLAP.SEGMENT4,GLAP.SEGMENT5,GLAP.ACCOUNT_NAME,GLAP.ACCT_CURR, GLAP.JE_SOURCE)

---GLO1_SUM-----------
,GLO1_SUM AS (
SELECT AOC_NAME, ACCT_PERIOD, ACCOUNT_ID, SUM(ACCT_AMT) AS ACCT_AMT FROM GLO1 GLO1
GROUP BY AOC_NAME, ACCT_PERIOD, ACCOUNT_ID
)
---GLAP1_SUM-----------
,GLAP1_SUM AS (
SELECT AOC_NAME, ACCT_PERIOD, ACCOUNT_ID, SUM(ACCT_AMT) AS ACCT_AMT FROM GLAP1 GLAP1
GROUP BY AOC_NAME, ACCT_PERIOD, ACCOUNT_ID
)
...
FROM APIL APIL
--WHERE APIL.ACCRUAL_PERIOD='000000-000'
GROUP BY APIL.AOC_NAME1, APIL.AOC_ID, APIL.ACCT_YEAR,
APIL.ACCT_PERIOD, APIL.ACCRUAL_PERIOD, APIL.ACCOUNT_ID, APIL.ACCOUNT_NAME, --APIL.TRX_CURR,APIL.ACCT_CURR,
APIL.SEGMENT4,APIL.SEGMENT5
)
---AP_ACCR_RPT---------------------------------------
,AP_ACCR_RPT AS (
SELECT --'AP_ACCR_RPT' AS T0,
R.AOC_NAME, R.ACCRUAL_ACCT_ID, R.ACCRUAL_ACCT_NAME, R.ACCRUAL_PERIOD, R.ACCRUAL_PERIOD_AP, R.SEGMENT4,R.SEGMENT5,R.SEGMENT5_ISNUM, R.JE_SOURCE,
R.ACCT_PERIOD, R.RNA，R.RND,
R.ACCT_CURR, R.ACCRRUAL_CR_AMT, R.ACTUAL_EXP_AMT,
R.CLOSE_BAL_ACCR_AMT,
CASE WHEN R.RND=1 AND R.RNA > 6 THEN 0
--WHEN R.CLOSE_BAL_ACCR_AMT <0 THEN 0
ELSE R.CLOSE_BAL_ACCR_AMT
END AS CLOSE_BAL_ACCR_AMT_ADJUSTED,
R.ACCR_ADJ__DR__CR,
CASE WHEN R.RND=1 AND R.RNA > 6 THEN 0
WHEN R.ACCR_ADJ__DR__CR <0 THEN 0 ELSE R.ACCR_ADJ__DR__CR END AS ACCR_ADJ__DR__CR_ADJUSTED,
R.EXPENSE_ACCT_ID, R.EXPENSE_ACCT_NAME
FROM
(
SELECT G.AOC_NAME,
G.ACCOUNT_ID AS ACCRUAL_ACCT_ID, G.ACCOUNT_NAME AS ACCRUAL_ACCT_NAME, G.ACCT_PERIOD, G.ACCRUAL_PERIOD, G.JE_SOURCE,
...
G.ACCT_AMT+NVL(A.ACCT_AMT,0) AS CLOSE_BAL_ACCR_AMT,
((G.ACCT_AMT+NVL(A.ACCT_AMT,0))-G.ACCT_AMT) AS ACCR_ADJ__DR__CR,
...
---AP_ACCR_RPT1---------------------------------------
,AP_ACCR_RPT1 AS (
SELECT --'AP_ACCR_RPT1' AS T0,
R.AOC_NAME, R.ACCRUAL_ACCT_ID, R.ACCRUAL_ACCT_NAME, R.ACCRUAL_PERIOD, R.ACCRUAL_PERIOD_AP, R.SEGMENT4,R.SEGMENT5,R.SEGMENT5_ISNUM,
R.JE_SOURCE,
R.ACCT_PERIOD,
R.RNA，R.RND,
R.ACCT_CURR,
...
FROM
(
SELECT CASE WHEN G.AOC_NAME IS NULL THEN A.AOC_NAME ELSE G.AOC_NAME END AS AOC_NAME,
G.ACCOUNT_ID AS ACCRUAL_ACCT_ID, G.ACCOUNT_NAME AS ACCRUAL_ACCT_NAME,
CASE WHEN G.ACCT_PERIOD IS NULL THEN A.ACCT_PERIOD ELSE G.ACCT_PERIOD END AS ACCT_PERIOD,
CASE WHEN G.ACCRUAL_PERIOD IS NULL THEN A.ACCRUAL_PERIOD ELSE G.ACCRUAL_PERIOD END AS ACCRUAL_PERIOD,
G.JE_SOURCE,
G.SEGMENT4,G.SEGMENT5,
LENGTH(TRIM(TRANSLATE(G.SEGMENT5, ' +-.0123456789',' '))) AS SEGMENT5_ISNUM,
G.RNA，G.RND,
CASE WHEN G.ACCT_CURR IS NULL THEN A.ACCT_CURR ELSE G.ACCT_CURR END AS ACCT_CURR,
NVL(G.ACCT_AMT,0) AS ACCRRUAL_CR_AMT,
NVL(A.ACCT_AMT,0) AS ACTUAL_EXP_AMT,
NVL(G.ACCT_AMT,0)+NVL(A.ACCT_AMT,0) AS CLOSE_BAL_ACCR_AMT,
((NVL(G.ACCT_AMT,0)+NVL(A.ACCT_AMT,0))-NVL(G.ACCT_AMT,0)) AS ACCR_ADJ__DR__CR,
A.ACCOUNT_ID AS EXPENSE_ACCT_ID, A.ACCOUNT_NAME AS EXPENSE_ACCT_NAME, A.ACCRUAL_PERIOD AS ACCRUAL_PERIOD_AP
FROM GLO1 G FULL OUTER JOIN GLAP1 A ON G.ACCT_PERIOD=A.ACCT_PERIOD AND G.ACCRUAL_PERIOD=A.ACCRUAL_PERIOD
--WHERE G.COMBO_IDX='Thai AirAsia Co Ltd-201904-SNODMK-DMK' OR A.COMBO_IDX='Thai AirAsia Co Ltd-201904-SNODMK-DMK'
)R)

,AP_ACCR_RPT2 AS (
SELECT R.AOC_NAME, R.ACCRUAL_ACCT_ID, R.ACCRUAL_ACCT_NAME, R.ACCRUAL_PERIOD, R.ACCRUAL_PERIOD_AP, R.SEGMENT4,R.SEGMENT5,R.SEGMENT5_ISNUM,
R.JE_SOURCE, R.ACCT_PERIOD, R.RNA，R.RND, R.ACCT_CURR,
R.ACCRRUAL_CR_AMT,
R.ACTUAL_EXP_AMT,
--
R.CLOSE_BAL_ACCR_AMT,
CASE WHEN R.ACTUAL_EXP_AMT > R.ACCRRUAL_CR_AMT THEN 0 ELSE R.CLOSE_BAL_ACCR_AMT_ADJUSTED END AS CLOSE_BAL_ACCR_AMT_ADJUSTED,
--
R.ACCR_ADJ__DR__CR,
CASE WHEN R.ACTUAL_EXP_AMT > R.ACCRRUAL_CR_AMT THEN R.ACCRRUAL_CR_AMT*-1 ELSE R.ACCR_ADJ__DR__CR_ADJUSTED END AS ACCR_ADJ__DR__CR_ADJUSTED,
R.EXPENSE_ACCT_ID, R.EXPENSE_ACCT_NAME
FROM AP_ACCR_RPT1 R)



--SELECT 'GLO_SUM' AS T, GLO_SUM.* FROM GLO_SUM GLO_SUM
--SELECT 'GLAP_SUM' AS T, GLAP_SUM.* FROM GLAP_SUM GLAP_SUM
--SELECT 'GLO1_SUM' AS T, GLO1_SUM.* FROM GLO1_SUM GLO1_SUM
--SELECT 'GLAP1_SUM' AS T, GLAP1_SUM.* FROM GLAP1_SUM GLAP1_SUM
--SELECT 'AP_ACCR_RPT' AS T0, AP_ACCR_RPT.* FROM AP_ACCR_RPT AP_ACCR_RPT
--SELECT 'AP_ACCR_RPT1' AS T0, AP_ACCR_RPT1.* FROM AP_ACCR_RPT1 AP_ACCR_RPT1
SELECT 'AP_ACCR_RPT2' AS T0, AP_ACCR_RPT2.* FROM AP_ACCR_RPT2 AP_ACCR_RPT2
/*SELECT 'GLAP1' AS T, GLAP1.COMBO_IDX, SUM(GLAP1.ACCT_AMT) AS ACCT_AMT FROM GLAP1 GLAP1
WHERE GLAP1.ACCT_PERIOD='201904'
GROUP BY GLAP1.COMBO_IDX
*/

/*SELECT 'GLO01_VS_GLAP1' AS T,
GLO1.ACCT_PERIOD AS GLO1_ACCT_PERIOD,
GLAP1.ACCT_PERIOD AS GLAP1_ACCT_PERIOD,
SUM(NVL(GLO1.ACCT_AMT,0)) AS ACCRUAL_AMT,
SUM(NVL(GLAP1.ACCT_AMT,0)) AS EXPENSE_AMT
FROM GLO1 GLO1 FULL OUTER JOIN GLAP1 GLAP1 ON GLO1.COMBO_IDX=GLAP1.COMBO_IDX
GROUP BY GLO1.ACCT_PERIOD, GLAP1.ACCT_PERIOD
*/
